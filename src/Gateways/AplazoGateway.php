...
namespace Aplazo\Woocommerce\Gateways;
...
use WP_Error;
use Aplazo\Woocommerce\Logs\Logs;

if ( ! defined( 'ABSPATH' ) ) {
    exit;
}

class AplazoGateway extends WC_Payment_Gateway
{
    ...
    private $_checkout_url = '/api/loan';
    private $_verify_url = '/api/auth';
    ...
    public function auth()
    {
        ...
        if ( is_wp_error( $response ) ) {
            ...
            $this->aplazo_sevice->sendLog('Error > ' . $error_message, Api::LOGS_CATEGORY_ERROR, Api::LOGS_SUBCATEGORY_AUTH);
            return false;
        } else {
            $response = json_decode(wp_remote_retrieve_body( $response ), true );
            $this->log->write_log('info', $response);
            $this->aplazo_sevice->sendLog('Auth > ' . json_encode($response), Api::LOGS_CATEGORY_INFO, Api::LOGS_SUBCATEGORY_AUTH);
            return $response;
        }
    }
            ...
            foreach($response as $index => $loan){
                if (isset($loan['status'])) {
                    if($loan['status'] === self::LOAN_SUCCESS_STATUS){
                        ...
                        $this->aplazo_sevice->sendLog($message, Api::LOGS_CATEGORY_INFO, Api::LOGS_SUBCATEGORY_ORDER,
                            ['loanId' => $loan['loanId']]);
                        return $loan;
                    }
    ...
    {
        return $this->product_detail_widget == "yes";
    }

    public function show_widget_shopping_cart()
    {
        return $this->shopping_cart_widget == "yes";
    }
    ...
    function check_aplazo_from_api_response()
    {
        ...
        $this->log->write_log('info', $data);
        $this->aplazo_sevice->sendLog('Webhook started from Aplazo', Api::LOGS_CATEGORY_INFO, Api::LOGS_SUBCATEGORY_WEBHOOK,
        ...
        if (isset($data['status']) && !empty($order_id)) {
            ...
            if ($status == 'New' || $status == 'NEW') {
                ...
                $this->log->write_log('info', $message);
                $this->aplazo_sevice->sendLog($message, Api::LOGS_CATEGORY_INFO, Api::LOGS_SUBCATEGORY_WEBHOOK,
                    ['orderId' => $order_id]);
                ...
                $this->aplazo_sevice->sendLog($message, Api::LOGS_CATEGORY_ERROR, Api::LOGS_SUBCATEGORY_WEBHOOK);
                wp_die($message);
    ...
    public function process_refund( $order_id, $amount = null, $reason = '' ) {
        ...
        $this->aplazo_sevice->sendLog('Proceso de reembolso', Api::LOGS_CATEGORY_INFO, Api::LOGS_SUBCATEGORY_REFUND,
            ['orderId' => $order_id]);

        if ( ! $this->can_refund_order( $order ) ) {
            $this->log->write_log('error', 'Rembolso fallido. Revisar logs.');
            $this->aplazo_sevice->sendLog('Rembolso fallido. Revisar logs.', Api::LOGS_CATEGORY_ERROR, Api::LOGS_SUBCATEGORY_REFUND,
                ['orderId' => $order_id]);
        ...
        if ( !$result  ) {
            $this->aplazo_sevice->sendLog('Refund communication failed', Api::LOGS_CATEGORY_ERROR, Api::LOGS_SUBCATEGORY_REFUND,
                ['orderId' => $order_id]);
    ...
    public function includes() {
        ...
        $this->log = new Logs( self::SOURCE_LOG, $debugMode);
        $this->aplazo_sevice = new Api($this->log, $this);
    }
